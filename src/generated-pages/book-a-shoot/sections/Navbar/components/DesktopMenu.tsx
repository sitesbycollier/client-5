export const DesktopMenu = () => {
  return (
    <div className="self-start caret-transparent block col-end-2 col-start-1 row-end-2 row-start-1 h-8 justify-self-start ml-[calc(50%_-_490px)] min-h-[auto] min-w-[auto] outline-[3px] pointer-events-auto relative no-underline w-[840px] mt-[25px] mb-[7px] left-[77px]">
      <nav
        aria-label="Site"
        className="bg-[position:0px_0px] caret-transparent flex flex-col h-full absolute no-underline w-full inset-0"
      >
        <ul className="bg-[position:0px_0px] caret-transparent h-full list-none absolute text-center no-underline text-nowrap w-full pl-0">
          <li className="bg-[position:0px_0px] box-border caret-transparent inline-block h-8 relative no-underline text-nowrap w-28">
            <a
              href="https://www.snapmagicmedia.com/"
              className="bg-[position:0px_0px] caret-transparent text-blue-700 inline-block h-full no-underline text-nowrap w-full"
            >
              <div className="bg-[position:0px_0px] caret-transparent no-underline text-nowrap pr-[15px]">
                <div className="bg-white bg-[position:0px_0px] shadow-[rgba(0,0,0,0.6)_0px_1px_4px_0px] caret-transparent absolute no-underline text-nowrap overflow-hidden rounded-[10px] left-0 right-[15px] inset-y-0">
                  <div className="bg-[url(data:image/png;base64,)] bg-top bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]">
                    <div className="bg-[url(data:image/png;base64,)] bg-bottom bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]"></div>
                  </div>
                </div>
                <div className="bg-[position:0px_0px] caret-transparent relative no-underline text-nowrap">
                  <p className="bg-[position:0px_0px] caret-transparent text-zinc-800 inline-block text-base leading-8 no-underline text-nowrap px-2.5 font-wfont_387fbb_f948eb222b064376a33aa3bcb7abad2e">
                    HOME
                  </p>
                </div>
              </div>
            </a>
          </li>
          <li className="bg-[position:0px_0px] box-border caret-transparent inline-block h-8 relative no-underline text-nowrap w-[175px]">
            <a
              href="https://www.snapmagicmedia.com/book-a-shoot"
              className="bg-[position:0px_0px] caret-transparent text-blue-700 inline-block h-full no-underline text-nowrap w-full"
            >
              <div className="bg-[position:0px_0px] caret-transparent no-underline text-nowrap px-[15px]">
                <div className="bg-zinc-300 bg-[position:0px_0px] shadow-[rgba(0,0,0,0.6)_0px_1px_4px_0px] caret-transparent absolute no-underline text-nowrap overflow-hidden rounded-[10px] inset-x-[15px] inset-y-0">
                  <div className="bg-[url(data:image/png;base64,)] bg-top bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]">
                    <div className="bg-[url(data:image/png;base64,)] bg-bottom bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]"></div>
                  </div>
                </div>
                <div className="bg-[position:0px_0px] caret-transparent relative no-underline text-nowrap">
                  <p className="bg-[position:0px_0px] caret-transparent text-gray-800 inline-block text-base leading-8 no-underline text-nowrap px-2.5 font-wfont_387fbb_f948eb222b064376a33aa3bcb7abad2e">
                    Book a Shoot
                  </p>
                </div>
              </div>
            </a>
          </li>
          <li className="bg-[position:0px_0px] box-border caret-transparent inline-block h-8 relative no-underline text-nowrap w-[167px]">
            <a
              href="https://www.snapmagicmedia.com/commercial"
              className="bg-[position:0px_0px] caret-transparent text-blue-700 inline-block h-full no-underline text-nowrap w-full"
            >
              <div className="bg-[position:0px_0px] caret-transparent no-underline text-nowrap px-[15px]">
                <div className="bg-white bg-[position:0px_0px] shadow-[rgba(0,0,0,0.6)_0px_1px_4px_0px] caret-transparent absolute no-underline text-nowrap overflow-hidden rounded-[10px] inset-x-[15px] inset-y-0">
                  <div className="bg-[url(data:image/png;base64,)] bg-top bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]">
                    <div className="bg-[url(data:image/png;base64,)] bg-bottom bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]"></div>
                  </div>
                </div>
                <div className="bg-[position:0px_0px] caret-transparent relative no-underline text-nowrap">
                  <p className="bg-[position:0px_0px] caret-transparent text-zinc-800 inline-block text-base leading-8 no-underline text-nowrap px-2.5 font-wfont_387fbb_f948eb222b064376a33aa3bcb7abad2e">
                    Commercial
                  </p>
                </div>
              </div>
            </a>
          </li>
          <li className="bg-[position:0px_0px] box-border caret-transparent inline-block h-8 relative no-underline text-nowrap w-[251px]">
            <a
              href="https://www.snapmagicmedia.com/photography"
              className="bg-[position:0px_0px] caret-transparent text-blue-700 inline-block h-full no-underline text-nowrap w-full"
            >
              <div className="bg-[position:0px_0px] caret-transparent no-underline text-nowrap px-[15px]">
                <div className="bg-white bg-[position:0px_0px] shadow-[rgba(0,0,0,0.6)_0px_1px_4px_0px] caret-transparent absolute no-underline text-nowrap overflow-hidden rounded-[10px] inset-x-[15px] inset-y-0">
                  <div className="bg-[url(data:image/png;base64,)] bg-top bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]">
                    <div className="bg-[url(data:image/png;base64,)] bg-bottom bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]"></div>
                  </div>
                </div>
                <div className="bg-[position:0px_0px] caret-transparent relative no-underline text-nowrap">
                  <p className="bg-[position:0px_0px] caret-transparent text-zinc-800 inline-block text-base leading-8 no-underline text-nowrap px-2.5 font-wfont_387fbb_f948eb222b064376a33aa3bcb7abad2e">
                    Photography A-La-Carte
                  </p>
                </div>
              </div>
            </a>
          </li>
          <li className="bg-[position:0px_0px] box-border caret-transparent inline-block h-8 relative no-underline text-nowrap w-[135px]">
            <a
              href="https://www.snapmagicmedia.com/contact"
              className="bg-[position:0px_0px] caret-transparent text-blue-700 inline-block h-full no-underline text-nowrap w-full"
            >
              <div className="bg-[position:0px_0px] caret-transparent no-underline text-nowrap pl-[15px]">
                <div className="bg-white bg-[position:0px_0px] shadow-[rgba(0,0,0,0.6)_0px_1px_4px_0px] caret-transparent absolute no-underline text-nowrap overflow-hidden rounded-[10px] left-[15px] right-0 inset-y-0">
                  <div className="bg-[url(data:image/png;base64,)] bg-top bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]">
                    <div className="bg-[url(data:image/png;base64,)] bg-bottom bg-repeat-x caret-transparent h-full no-underline text-nowrap w-full rounded-[10px]"></div>
                  </div>
                </div>
                <div className="bg-[position:0px_0px] caret-transparent relative no-underline text-nowrap">
                  <p className="bg-[position:0px_0px] caret-transparent text-zinc-800 inline-block text-base leading-8 no-underline text-nowrap px-2.5 font-wfont_387fbb_f948eb222b064376a33aa3bcb7abad2e">
                    CONTACT
                  </p>
                </div>
              </div>
            </a>
          </li>
        </ul>
        <div className="bg-[position:0px_0px] caret-transparent hidden no-underline">
          Use tab to navigate through the menu items.
        </div>
      </nav>
    </div>
  );
};
